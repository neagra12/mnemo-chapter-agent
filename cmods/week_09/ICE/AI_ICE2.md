...
**Critical Prerequisite:** This workshop is **100% dependent** on the **"Week 9 ICE Prep"** assignment. The `Dev Crew` member (and ideally everyone) must have their self-hosted runner active *right now*.
  ...
  * **To start your runner:**
    ...
    3.  You should see it output: `√ Connected to GitHub` and `...Listening for Jobs`.
...
### Part 2: The "Happy Path" (Get to Green ✅)
  ...
  * **([Process Lead])**
    ...
    2.  **Verification:** The team's goal is to see a **green checkmark ✅**. This proves the pipeline is configured correctly.
    3.  **Announce to the Dev Crew:** "We have a green check\! Process is validated. Proceed with the fire drill\!"

### Part 3: The "Fire Drill" (Get to Red ❌)
  ...
  * **([Process Lead])**
    ...
    2.  **Verification:** The team's goal is to see a **Red X ❌**.
    3.  **Announce to the team:** "Fire\! Fire\! The build is red\!"

### Part 4: The Code Review (Rejecting the PR 🙅‍♂️)
  ...
  * **([Process Lead])**
    ...
    5.  Create the PR. Notice the **Red X ❌** on the PR.
  ...
  * **([Repo Admin])**
    ...
    4.  Select the **"Request Changes" 🙅‍♂️** option and submit your review.
    5.  **Announce to the Dev Crew:** "PR has been formally 'Rejected'. Fix the build."

### Part 5: The Fix (Back to Green ✅)
  ...
  * **([Process Lead])**
    ...
    2.  Watch as the `Dev Crew`'s push *automatically* triggers a new CI run, which updates to a **Green Check ✅**.
    3.  **Announce to the Repo Admin:** "The build is green\! Re-review, please."

### Part 6: The Approval (Approving the PR 👍)

  * **([Repo Admin])**
    1.  Go back to the PR. You will now see a **Green Check ✅**.
    2.  Click **"Review changes"** again. (You may need to "Dismiss" your old review first).
    3.  Select the **"Approve" 👍** option and submit your review. **DO NOT MERGE.**
...
* **Summary of In-Class Work:** We successfully validated our team's CI/CR process.
    ...
    4.  The entire team commented, and the `Repo Admin` formally **"Requested Changes" 🙅‍♂️**.
...
Your team's in-class workshop is "Done" when you can show a TA all of the following:
  ...
  * [ ] **The "In-Class Demo PR"** that is **"Approved" 👍** and has a **"Green Check" ✅**.
  * [ ] The demo PR's history *also* shows a **"Red X" ❌** and the **"Requested Changes" 🙅‍♂️** event.
...
1.  **Individual Task (Everyone):**
      ...
      * **Reviewer:** Go to your partner's PR and **"Request Changes" 🙅‍♂️**.
      * **Author:** Push the *fix* to your branch (gets a **Green Check ✅**).
      * **Reviewer:** Go back and **"Approve" 👍** the PR.
...
## TA Follow-up Guide
  ...
  * **In-Class Goal (Workshop):** Guide *one* team member (`Dev Crew`) through the full "Green $\rightarrow$ Red $\rightarrow$ Green" loop, with the *entire team* participating in the CR. Your goal is to **validate the *process*** so they can do it individually for homework.

  * **Final DoD (Due 11:59 PM):** The team submits *one* PR URL, which contains evidence of the in-class demo *and* links to every member's individual homework PR.

  * **Grading Rubric (10 points):** (See table above)

  * **Common Pitfalls & Coaching:**

    1.  **Job is "Queued" indefinitely (Blocker 1a):**
          * **Symptom:** The Action has a yellow "Queued" icon in Part 2 and never starts.
          * **The Fix:** **This is the \#1 blocker.** It means the `Dev Crew` member does not have their runner active. Have them start their `./run.sh` script.
    2.  **`pytest: command not found` (Blocker 1b):**
          * **Symptom:** The *first* CI run fails at the "Run tests with pytest" step.
          * **The Fix:** This means `pytest` is not in their `requirements.txt` file from ICE 1. The `Dev Crew` must: 1. Add `pytest` to `requirements.txt`. 2. `git add .` 3. `git commit ...` 4. `git push`. This will trigger a *new* run.
    3.  **PR Has No "Reviewers" Option:**
          * **Symptom:** The `Process Lead` (Part 4) can't assign the `Repo Admin` as a reviewer.
          * **The Fix:** The `Repo Admin` (or other member) never accepted the collaborator invite from ICE 1. They must find the email invite and click "Accept."
    4.  **"Approve" Button is Greyed Out:**
          * **Symptom:** The `Repo Admin` (Part 6) can't approve the PR, even after the check is green.
          * **The Fix:** The `Repo Admin` must first *dismiss* their own "Request Changes" review.

  * **Coaching Questions (for next team meeting):**

      * "How did it feel to see the 'Red X' on your PR? How did the 'Request Changes' feature change the team's communication from 'you broke it' to 'this needs a fix'?"
      * "Your homework required *everyone* to do this. Why was that important? What's the value in *everyone* knowing how to read a failed build?"

  * **Feed-Forward Prompts (to prep for the next ICE):**

      * "You've now automated a check for *correctness* (it passes `pytest`). But you can still push ugly, unreadable code. In our next lecture, we'll add a 'Linting' job to our pipeline to automatically check for code *quality*."

-----

### Pedagogical Analysis

  * **Core Goal (SE Experience):** Simulates the *complete* professional loop: (1) a team validates a new process, and (2) all team members are individually trained and certified on that process.
  * **Developer Workflow Competency:** This is a world-class exercise. It teaches the full CI/CR loop: `push` $\rightarrow$ `fail (Red X)` $\rightarrow$ `open PR` $\rightarrow$ `request changes (CR)` $\rightarrow$ `push fix` $\rightarrow$ `pass (Green ✅)` $\rightarrow$ `approve (CR)`.
  * **Ethical Challenge:** Not applicable for this technical exercise.
  * **Historical Context:** Not applicable for this exercise.
  * **Creative Engagement:** The "Fire Drill" narrative is a perfect, high-stakes-but-safe framing for intentionally breaking the build. The "Silliness Detector" and "A silly place" (a Python-specific Easter Egg) reinforce the MoJ theme.
  * **Cognitive Load Management:**
    1.  **"Needle-Thread" Model:** This plan is the optimal synthesis. It uses the 45-min **in-class** time for a TA-supported *team demo* to validate the *process*. This de-risks the **homework**, which scales the *individual competency* to all members.
    2.  **Sequential Scaffolding:** The "Green $\rightarrow$ Red $\rightarrow$ Green" loop is a brilliant piece of scaffolding. It proves the pipeline *works* (Happy Path) before asking students to *break* it (Failure Path).
    3.  **Full Team Engagement:** This version is superior as it solves the "idle member" problem. By requiring *all* members to comment on the PR (Part 4), it forces the entire team to actively participate in the in-class "fire drill" and diagnosis.