# Session for generating lecture 10 - External Security and 12 Factor App

## **---- PROMPT ----**
```markdown
Please generate lecture 9.
```

Here is the plan for Lecture 9, which closes out our "Security & Architecture" sprint.

This lecture "pays off" the technical debt of our `SECRET_KEY`, uses it as a hook to introduce the 12-Factor App, and then covers the two most common external web threats (XSS and CSRF), explaining how our framework is already protecting us.
...
## **---- PROMPT ----**
```markdown
This seems pretty easy on time for both. I would like to add a slide that talks about security scanners. We have our internal security tests but our app should have external scans by dedicated tools looking for vulnerabilities like the OWASP top 10.
```

You're right, both the lecture and ICE are a bit light on time, and that's the perfect place to add this topic. It's a professional-level concept that fits perfectly.

Here is the new slide, which would fit well right after the "Testing for Security" slide and before the "Key Takeaways."
